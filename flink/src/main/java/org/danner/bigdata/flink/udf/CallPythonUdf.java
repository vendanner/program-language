...
/**
 * Java 调用 python udf
 * https://github.com/pyflink/playgrounds/blob/master/examples/table/java/src/main/java/BlinkBatchPythonUdfSqlJob.java
 */
public class CallPythonUdf {
    public static void main(String[] args) {
        ...
        tEnv.getConfig().getConfiguration().set(PythonOptions.PYTHON_FILES, "/tmp/udfs.zip");
...
//        Iterator<Row> result = tEnv.executeSql("select add_one(a) as a from source").collect();
        Iterator<Row> result = tEnv.executeSql("select sa from source, LATERAL TABLE(add_one(a)) as T(sa)").collect();
